              ...
              getProduct.map((item, index) => (
                <Link key={index} to={`/product/${item.id_product}`} className={`${styles.links}`}>
                  <div className="card ms-4" style={{ width: '14rem', top: '10px' }}>
                    <img src={`${item.photo.split('||')[0].split('|&&|')[0]}`} height="200px" className="card-img-top" alt="..." />
                    <div className="card-body">
                      <h4 className="card-text">{item.product_name}</h4>
                      <h4 className="card-text text-danger">Rp. {item.price?.toString().split('').reverse().join('').match(/.{1,3}/g).join('.').split('').reverse().join('')}</h4>
